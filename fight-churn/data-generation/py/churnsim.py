...
class ChurnSimulation:
    ...
    def simulate_customer(self, start_of_month):
        ...
        customer_model = np.random.choice(
            self.model_list, p=self.population_percents["pcnt"]
        )
        new_customer = customer_model.generate_customer(start_of_month)

        customer_country = np.random.choice(
            self.country_lookup["country"], p=self.country_lookup["pcnt"]
        )
        ...
        while not churned:
            ...
            churned = (
                self.util_mod.simulate_churn(month_count, new_customer)
                or next_month > self.end_date
            )
            if not churned:
    ...
    def create_customers_for_month(self, month_date, n_to_create):
        ...
        for i in range(n_to_create):
            ...
            if i % 100 == 0:
                print(
                    "Simulated customer {}/{}: {:,} subscriptions & {:,} events".format(
                        i, n_to_create, total_subscriptions, total_events
                    )
                )
    ...
    def copy_customer_to_database(self, customer):
        ...
        sql = "INSERT INTO {}.account VALUES({},'{}','{}',{})".format(
            ...
            "NULL" if customer.country == "None" else "'{}'".format(customer.country),
        )
        self.db.run(sql)
        ...
        sql = (
            "COPY %s.subscription FROM STDIN USING DELIMITERS ',' WITH NULL AS '\\null'"
            % (self.model_name)
        )
    ...
    def truncate_old_sim(self):
        ...
        if oldEvent > 0 or oldSubs > 0 or oldAccount > 0:
            ...
            if (
                input("are you sure? (enter %s to proceed) " % self.model_name)
                == self.model_name
            ):
                if oldEvent > 0:
    ...
    def run_simulation(self):
        ...
        self.behavior_models[next(iter(self.behavior_models))].insert_event_types(
            self.model_name, self.db
        )
        ...
        n_to_add = int(
            ceil(self.init_customers * self.monthly_growth_rate)
        ...
        while next_month < self.end_date:
            ...
            n_to_add = int(
                ceil(n_to_add * (1.0 + self.monthly_growth_rate))
