...
class UtilityModel:
    ...
    def setChurnScale(self, bemodDict, model_weights):
        ...
        assert self.expected_utility > 0, (
            "Print model requires utility >0, instead expected utility is %f"
    ...
    def utility_function(self, event_counts, customer):
        ...
        utility_contribs = self.expected_contributions * (
            1.0 - np.exp(-2.0 * contrib_ratios)
        )
        utility = np.sum(utility_contribs)
        if customer.satisfaction_propensity != 1.0:
            multiplier = (
                customer.satisfaction_propensity
                if utility > 0.0
                else (1.0 / customer.satisfaction_propensity)
            )
        else:
            multiplier = 1.0
        utility *= multiplier
